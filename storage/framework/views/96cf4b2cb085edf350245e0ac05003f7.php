            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-2" style="color: #333; font-weight: 600;">
                        <i class="fas fa-building text-primary me-2"></i><?php echo e($theater->name); ?>

                    </h5>
                    <p class="text-muted mb-1"><i class="fas fa-map-marker-alt"></i> <?php echo e($theater->location ?? 'N/A'); ?></p>
                    <?php if($theater->address): ?>
                        <p class="text-muted mb-1"><i class="fas fa-address-card"></i> <?php echo e($theater->address); ?></p>
                    <?php endif; ?>
                    <?php if($theater->phone): ?>
                        <p class="text-muted mb-0"><i class="fas fa-phone"></i> <?php echo e($theater->phone); ?></p>
                    <?php endif; ?>
                </div>
                <div class="text-end">
                    <span class="badge bg-success fs-6 px-3 py-2">
                        <i class="fas fa-door-open me-1"></i><?php echo e($theater->total_screens); ?> phòng chiếu
                    </span>
                </div>
            </div>
        </div>
...
<div class="row mb-4">
    ...
    <div class="col-md-3">
        <div class="stat-card animated-card" data-delay="<?php echo e($loop->index * 100 + 100); ?>">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <div class="stat-label"><?php echo e($stat['label']); ?></div>
                    <div class="stat-value text-<?php echo e($stat['color']); ?> counter" data-target="<?php echo e($stat['value']); ?>">0</div>
                </div>
                <div class="stat-icon bg-<?php echo e($stat['color']); ?>"><i class="fas fa-<?php echo e($stat['icon']); ?>"></i></div>
            </div>
            <div class="stat-progress"><div class="progress-bar bg-<?php echo e($stat['color']); ?>" style="width:100%"></div></div>
        </div>
    </div>
        ...
        <div class="stat-card animated-card revenue-card" data-delay="500">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <div class="stat-value text-success revenue-counter" data-target="<?php echo e($stats['total_revenue']); ?>">0₫</div>
                </div>
        ...
        <div class="stat-card animated-card revenue-card" data-delay="600">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <div class="stat-value text-primary revenue-counter" data-target="<?php echo e($stats['today_revenue']); ?>">0₫</div>
                </div>
                    ...
                    <li class="mb-3 pb-2 border-bottom movie-item" style="animation-delay: <?php echo e($index * 100); ?>ms;">
                        <div class="d-flex justify-content-between align-items-start mb-1">
                            <div class="d-flex align-items-center flex-grow-1">
                                ...
                                <span class="flex-grow-1"><?php echo e($movie['title']); ?></span>
                            </div>
                            <span class="badge bg-info"><?php echo e($movie['ticket_count']); ?> vé</span>
                        </div>
                        <div class="ms-4">
                            <small class="text-success fw-bold"><i class="fas fa-dollar-sign me-1"></i><?php echo e(number_format($movie['revenue'] ?? 0)); ?>₫</small>
                        </div>
                    </li>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </ul>
            ...
            <?php else: ?>
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            <?php $currentDate = ''; ?>
                            <?php $__currentLoopData = $upcomingShowtimes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $showtime): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php
                                    $isToday = $showtime['show_date'] === date('Y-m-d');
                                    $isTomorrow = $showtime['show_date'] === date('Y-m-d', strtotime('+1 day'));
                                    $showDate = $showtime['show_date'];
                                ?>
                                <?php if($currentDate !== $showDate): ?>
                                    <?php $currentDate = $showDate; $dateLabel = $isToday ? 'Hôm nay' : ($isTomorrow ? 'Ngày mai' : date('d/m/Y', strtotime($showDate))); ?>
                                    <tr class="table-secondary">
                                        <td colspan="8" class="fw-bold py-2">
                                            <i class="fas fa-calendar-day me-2"></i><?php echo e($dateLabel); ?>

                                            <?php if($isToday): ?><span class="badge bg-success ms-2">Hôm nay</span><?php endif; ?>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                                <tr class="showtime-row <?php echo e($isToday ? 'table-warning-light' : ''); ?>">
                                    <td>
                                        <?php if(!empty($showtime['thumbnail'])): ?>
                                            <img src="<?php echo e($showtime['thumbnail']); ?>" alt="<?php echo e($showtime['movie_title']); ?>" class="rounded" style="width:40px;height:55px;object-fit:cover;">
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <div class="fw-semibold"><?php echo e($showtime['movie_title']); ?></div>
                                        <?php if(!empty($showtime['duration'])): ?><small class="text-muted"><?php echo e($showtime['duration']); ?> phút</small><?php endif; ?>
                                    </td>
                                    <td><span class="<?php echo e($isToday ? 'text-success fw-bold' : ''); ?>"><?php echo e(date('d/m', strtotime($showtime['show_date']))); ?></span></td>
                                    <td><span class="badge bg-primary fs-6"><?php echo e(date('H:i', strtotime($showtime['show_time']))); ?></span></td>
                                    <td>
                                        <span class="badge bg-<?php echo e($showtime['screen_type'] === '3D' ? 'danger' : 'secondary'); ?>"><?php echo e($showtime['screen_name']); ?></span>
                                        <small class="d-block text-muted"><?php echo e($showtime['screen_type']); ?></small>
                                    </td>
                                    <td>
                                        <?php
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="progress flex-grow-1 me-2" style="height:8px;width:60px;">
                                                <div class="progress-bar <?php echo e($progressClass); ?>" style="width:<?php echo e($percentage); ?>%"></div>
                                            </div>
                                            <small class="text-nowrap"><?php echo e($ticketsSold); ?>/<?php echo e($totalSeats); ?></small>
                                        </div>
                                    </td>
                                    <td><span class="text-success fw-bold"><?php echo e(number_format($showtime['price'])); ?>₫</span></td>
                                    <td class="text-center">
                                        <a href="?route=moderator/showtimes&date=<?php echo e($showtime['show_date']); ?>" class="btn btn-sm btn-outline-info" title="Xem chi tiết">
...
<?php echo $__env->make('admin.moderator.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\CineHub_DuAnTotNghiep\resources\views/admin/moderator/dashboard.blade.php ENDPATH**/ ?>