...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h5>Quản lý phòng chiếu - {{ $theater['name'] }}</h5>
        ...
        <div class="col-md-4">
            ...
            <select name="movie_id" id="movie_filter" class="form-select" onchange="this.form.submit()">
                ...
                @foreach($movies as $movie)
                    <option value="{{ $movie['id'] }}" {{ (isset($_GET['movie_id']) && $_GET['movie_id'] == $movie['id']) ? 'selected' : '' }}>
                        {{ $movie['title'] }}
                    </option>
                @endforeach
            </select>
            ...
            <tbody>
                ...
                @else
                    ...
                    <tr>
                        ...
                        <td>{{ $screen['total_seats'] }} ghế</td>
                        <td>
                            @if(!empty($screen['current_movies']) && is_array($screen['current_movies']) && count($screen['current_movies']) > 0)
                                <div class="d-flex flex-column gap-1">
                                    @foreach($screen['current_movies'] as $movie)
                                        <span class="badge bg-primary">{{ $movie['title'] ?? '' }}</span>
                                    @endforeach
                                </div>
                            @else
