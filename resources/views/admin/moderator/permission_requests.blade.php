                ...
                <div class="stat-card">
                    ...
                    <div class="list-group">
                        @foreach($requests as $req)
                            <a href="?route=moderator/permissionRequests&id={{ $req['id'] }}"
                                class="list-group-item list-group-item-action {{ ($selectedRequest && $selectedRequest['id'] == $req['id']) ? 'active' : '' }}">
                                <div class="d-flex justify-content-between align-items-start">
                                    <div>
                                        <h6 class="mb-1">{{ $req['target_user_name'] }}</h6>
                                        <small class="text-muted">Yêu cầu bởi: {{ $req['requested_by_name'] }}</small>
                                        <br><small class="text-muted">{{ date('d/m/Y H:i', strtotime($req['created_at'])) }}</small>
                                    </div>
                ...
                @if($selectedRequest)
                    ...
                    <div class="stat-card">
                        ...
                        <div class="mb-3">
                            ...
                            <p class="fw-bold">{{ $selectedRequest['target_user_name'] }}</p>
                            <small class="text-muted">{{ $selectedRequest['target_user_email'] }}</small>
                        </div>
                        <div class="mb-3">
                            <label class="form-label text-muted">Yêu cầu bởi:</label>
                            <p class="fw-bold">{{ $selectedRequest['requested_by_name'] }}</p>
                            <small class="text-muted">{{ $selectedRequest['requested_by_email'] }}</small>
                        </div>
                        <div class="mb-3">
                            <label class="form-label text-muted">Rạp:</label>
                            <p class="fw-bold">{{ $selectedRequest['theater_name'] }}</p>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label class="form-label text-muted">Quyền hiện tại:</label>
                                <p class="fw-bold">
                                    <span class="badge bg-secondary">{{ $oldData['role'] ?? 'user' }}</span>
                                    @if(isset($oldData['theater_id']) && $oldData['theater_id'])
                                        <br><small class="text-muted">Rạp ID: {{ $oldData['theater_id'] }}</small>
                                    @endif
                                </p>
                            </div>
                            <div class="col-md-6">
                                ...
                                <p class="fw-bold">
                                    <span class="badge bg-primary">{{ $newData['role'] ?? 'user' }}</span>
                                    @if(isset($newData['theater_id']) && $newData['theater_id'])
                                        <br><small class="text-muted">Rạp ID: {{ $newData['theater_id'] }}</small>
                                    @endif
                                </p>
                            </div>
                        </div>
                        <div class="mb-3">
                            ...
                            <p>{{ date('d/m/Y H:i:s', strtotime($selectedRequest['created_at'])) }}</p>
                        </div>
