...
document.getElementById('manualVerifyBtn').addEventListener('click', function() {
    ...
    if (code) {
        ...
        alert('Vui lòng nhập booking code hoặc booking ID');
    }
});

document.getElementById('manualCode').addEventListener('keypress', function(e) {
    if (e.key === 'Enter') {
        document.getElementById('manualVerifyBtn').click();
    }
});
...
function verifyTicket(code) {
    // Parse code để lấy booking_id hoặc booking_code
    let booking_id = null;
    let booking_code = null;
    
    // Kiểm tra xem code có phải là URL không
...
function displayTicketInfo(data) {
    ...
    let html = `
        <div class="card mb-3">
            <div class="card-header bg-primary text-white">
                <strong>Booking Code: ${booking.booking_code}</strong>
            </div>
            <div class="card-body">
    ...
    tickets.forEach(ticket => {
        ...
        html += `
            <li class="list-group-item d-flex justify-content-between align-items-center">
                Ghế ${ticket.seat} - ${ticket.seat_type}
                ${pickedUp}
            </li>
        `;
    });
    ...
    html += `
                ...
                <p class="mt-3"><strong>Tổng số vé:</strong> ${tickets.length}</p>
                <p><strong>Đã xác nhận:</strong> ${data.updated_count} vé</p>
            </div>
        </div>
    `;
