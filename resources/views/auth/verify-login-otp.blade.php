...
<head>
    ...
    <title>Xác thực đăng nhập OTP - CineHub</title>
    <!-- Bootstrap 5 CDN -->
...
<body>
    ...
    @php
        ...
        if (session()->has('login_user_id')) {
            $user = \App\Models\User::find(session('login_user_id'));
            if ($user) {
    ...
    <div class="otp-container">
        ...
        <div>
            <i class="fa-solid fa-user-shield otp-icon"></i>
        </div>
        <h2>Xác thực đăng nhập</h2>
        <p class="description">Để bảo mật tài khoản, vui lòng nhập mã xác thực gồm 6 chữ số đã được gửi tới email <strong class="text-white">{{ $userEmail }}</strong>.</p>
        ...
        <form action="{{ route('auth.verify-login-otp') }}" method="POST">
            @csrf
            <div class="mb-4">
                <input type="text" 
                       name="otp" 
            ...
            <button type="submit" class="btn btn-verify w-100 py-3">
                <i class="fa-solid fa-key me-2"></i>Xác nhận đăng nhập
            </button>
        </form>

        <div class="timer-container">
            <span id="countdown-text">Mã OTP đăng nhập sẽ hết hạn sau: <span id="timer">05:00</span></span>
    ...
    <script>
        ...
        function resendOtp() {
            ...
            fetch("{{ route('auth.resend-login-otp') }}", {
                method: "POST",
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert(data.message || 'Mã OTP đăng nhập mới đã được gửi!');
