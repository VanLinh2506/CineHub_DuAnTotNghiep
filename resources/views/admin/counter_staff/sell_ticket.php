...
<div class="sell-ticket-container">
    ...
    <div class="showtime-selection">
        <div class="form-group">
            ...
            <input type="date" id="selectDate" value="<?php echo $date; ?>" min="<?php echo date('Y-m-d'); ?>" class="form-control">
        </div>
        ...
        <div class="showtimes-grid" id="showtimesGrid">
            ...
            <?php else: ?>
                <?php foreach ($showtimes as $st): ?>
                    <div class="showtime-card <?php echo ($selected_showtime && $selected_showtime['id'] == $st['id']) ? 'selected' : ''; ?>" 
                         data-showtime-id="<?php echo $st['id']; ?>"
                         onclick="selectShowtime(<?php echo $st['id']; ?>)">
                        <div class="movie-info">
                            <img src="<?php echo htmlspecialchars($st['thumbnail']); ?>" alt="<?php echo htmlspecialchars($st['movie_title']); ?>">
                            <div class="details">
                                <h4><?php echo htmlspecialchars($st['movie_title']); ?></h4>
                                <p><i class="fas fa-clock"></i> <?php echo date('H:i', strtotime($st['show_time'])); ?></p>
                                <p><i class="fas fa-tv"></i> <?php echo htmlspecialchars($st['screen_name']); ?></p>
                                <p><i class="fas fa-chair"></i> Còn <?php echo $st['available_seats']; ?> ghế</p>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
    ...
    <div class="seat-selection-section">
        <h3>Chọn ghế - <?php echo htmlspecialchars($selected_showtime['movie_title']); ?> 
            (<?php echo date('H:i', strtotime($selected_showtime['show_time'])); ?>)</h3>
            ...
            <div class="seat-row">
                ...
                <div class="seats">
                    ...
                    <div class="seat <?php echo $seatClass; ?>" 
                         ...
                         data-type="<?php echo $isCouple ? 'couple' : ($isVip ? 'vip' : 'normal'); ?>"
                         <?php if (!$isBooked): ?>onclick="toggleSeat(this)"<?php endif; ?>>
                        <?php echo $i; ?>
                    </div>
                    <?php endfor; ?>
                </div>
...
.showtime-card .details h4 {
    margin: 0 0 10px 0;
    color: #fff;
    font-size: 16px;
}

.showtime-card .details p {
    margin: 5px 0;
    color: #aaa;
    font-size: 14px;
}
...
<script>
let selectedSeats = [];
let showtimeId = <?php echo $selected_showtime ? $selected_showtime['id'] : 'null'; ?>;
let prices = {
    normal: <?php echo $selected_showtime ? $selected_showtime['price'] : 0; ?>,
    vip: <?php echo $selected_showtime ? ($selected_showtime['price'] * 1.5) : 0; ?>,
    couple: <?php echo $selected_showtime ? ($selected_showtime['price'] * 2.5) : 0; ?>
};
...
function toggleSeat(element) {
    ...
    if (element.classList.contains('selected')) {
        ...
        selectedSeats.push({ seat: seat, type: type });
    }
...
function updateSummary() {
    const seatsText = selectedSeats.length > 0 ? selectedSeats.map(s => s.seat).join(', ') : 'Chưa chọn';
...
function processSale() {
    ...
    if (!confirm('Xác nhận bán ' + selectedSeats.length + ' vé?')) {
        return;
    }
